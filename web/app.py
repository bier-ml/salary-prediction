...
def run_server():
    ...
    with col2:
        schedule = st.selectbox(label='Select schedule',
                                options=['полный рабочий день',
                                         'частичная занятость',
                                         'удаленная работа',
                                         'сменный график',
                                         'свободный график',
    ...
    with col3:
        city = st.selectbox(label='Select city_id',
                            options=[1, 57, 2, 102, 174])

    data = pd.DataFrame({'custom_position': [job_name],
                         'schedule': [schedule],
                         'city_id': [city]})
